...
async function startBot() {
  ...
  if (!client.authState.creds.registered) {
    const phoneNumber = await question(
      chalk.bgYellowBright.blueBright.bold(`WARNING`) + " " + chalk.yellow("Ingrese su número de WhatsApp\n") +
      chalk.bgBlue.white.bold(`INFO`) + " " + chalk.white("Ejemplo: 57301××××××") +
      chalk.yellow('\n---> ')
    )
    try {
