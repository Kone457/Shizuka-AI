...
let handler = async (m, { conn, text }) => {
  try {
    ...
    if (!mentioned || mentioned === sender) {
      caption = `😖 ${senderName} está teniendo un momento muy vergonzoso... alguien que lo saque de ahí 🫠`;
    } else {
      let targetName = await conn.getName(mentioned);
      if (!targetName) targetName = '@' + mentioned.split('@')[0];
      caption = `🫣 ${senderName} se avergonzó por culpa de ${targetName} ¡qué momento! 😬`;
    }
