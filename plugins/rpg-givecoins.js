...
if (!isNumber(args[0]) && args[0].startsWith('@')) return m.reply(`✿ Primero indica la cantidad que deseas transferir, seguido de la persona a quien se lo envías.\nEjemplo: *${usedPrefix + command} 1000 @mencion*`)
if (!who) return m.reply(`✿ Debes mencionar a alguien para transferir *${currency}*.`)
let user = getUser(global.db.data.users, m.sender)
let recipient = getUser(global.db.data.users, who)
if (!recipient) return m.reply(`✿ El usuario no está en la base de datos.`)
...
if (user.bank < count) return m.reply(`✿ No tienes suficientes *${currency}* en el banco para transferir.`)
user.bank -= count
if (typeof recipient.bank !== 'number') recipient.bank = 0
recipient.bank += count   
if (isNaN(user.bank)) user.bank = 0
let name = typeof recipient.name === 'string' && recipient.name.trim() && recipient.name.trim() !== '[object Promise]' ? recipient.name.trim() : ''
