let handler = async (m, { conn, args, command, isOwner }) => {
  ...
  if (!('welcome' in chatData))
    chatData.welcome = true;

  if (!('antiLink' in chatData))
    chatData.antiLink = true;

  if (!('antilinks' in chatData))
    chatData.antilinks = true;

  if (!('antiprivado' in chatData))
    chatData.antiprivado = true;

  if (!('economy' in chatData))
    chatData.economy = true;

  if (!('rpg' in chatData))
    chatData.rpg = true;

  if (!('gacha' in chatData))
    chatData.gacha = true;

  if (!('level' in chatData))
    chatData.level = true;

  if (!('adminonly' in chatData))
  ...
  if (!('alerts' in chatData))
    chatData.alerts = true;

  if (!('notprefix' in chatData))
    chatData.notprefix = false;

  if (!('jadibotmd' in botSettings))
