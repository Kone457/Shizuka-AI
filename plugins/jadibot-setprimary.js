...
let handler = async (m, { conn, usedPrefix, args }) => {
  try {
    if (!args[0] && !(await m.mentionedJid).length && !m.quoted) {
      return m.reply(`《✧》 Menciona al bot o responde a su mensaje.`);
    }

    const users = global.conns && Array.isArray(global.conns)
      ? global.conns.filter(
          conn => conn?.user && conn?.ws?.socket && conn.ws.socket.readyState === ws.OPEN
        )
      : [];
    ...
    if (
      ...
      (global.conn && botJid === global.conn.user.jid)
    ) {
      selectedBot = conn;
    } else {
      selectedBot = users.find(
        c => c.user && (c.user.jid === botJid || c.user.id === botJid)
      );
    }
    ...
    if (!selectedBot) {
      return conn.reply(
        m.chat,
        `《✧》 @${botJid.split("@")[0]} no es un bot de la misma sesión. Verifica los bots conectados usando *#bots*.`,
        m,
        { mentions: [botJid] }
      );
    }
    ...
    if (!chat) {
      return conn.reply(
        m.chat,
        "《✧》 No se pudo acceder a la base de datos del grupo.",
        m
      );
    }
    ...
    if (chat.primaryBot === botJid) {
      return conn.reply(
        m.chat,
        `《✧》 @${botJid.split("@")[0]} ya es el bot primario en este grupo.`,
        m,
        { mentions: [botJid] }
      );
    }
