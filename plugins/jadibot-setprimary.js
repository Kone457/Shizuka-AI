...
let handler = async (m, { conn, usedPrefix, args }) => {
  try {
    if (!args[0] && !m.quoted) {
      return m.reply(`️《✧》 Menciona el número de un bot o responde al mensaje.`);
    }
    ...
    if (!selectedBot) {
      return conn.reply(m.chat, `《✧》 @${botJid.split("@")[0]} no es un bot de la misma sesión. Verifica los bots conectados usando *#bots*.`, m, { mentions: [botJid] });
    }
    ...
    if (chat.primaryBot === botJid) {
      return conn.reply(m.chat, `《✧》 @${botJid.split("@")[0]} ya es el bot primario en este grupo.`, m, { mentions: [botJid] });
    }
