...
if (!user.bank) return m.reply(`✿ No tienes suficientes *${currency}* en el Banco.`)
if (user.bank < count) return m.reply(`✿ Solo tienes *${user.bank.toLocaleString()} ${currency}* en el Banco.`)
user.bank -= count * 1
user.coin += count * 1
await m.reply(`✿ Retiraste *${count.toLocaleString()} ${currency}* del banco, ahora podras usarlo pero tambien podran robartelo.`)}
