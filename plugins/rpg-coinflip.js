...
if (isNaN(cantidad)) return conn.reply(m.chat, `✿ Cantidad inválida, ingresa un número válido.\n> Ejemplo » *${usedPrefix + command} 200 cruz*`, m)
if (Math.abs(cantidad) < 100) return conn.reply(m.chat, `✿ La cantidad mínima para apostar es *100 ${currency}*.`, m)
if (!['cara', 'cruz'].includes(eleccion)) return conn.reply(m.chat, `✿ Elección inválida. Solo se admite *cara* o *cruz*.\n> Ejemplo » *${usedPrefix + command} 200 cara*`, m)
if (cantidad > user.coin) return conn.reply(m.chat, `✿ No tienes suficientes *${currency}* para apostar, tienes *${user.coin.toLocaleString()} ${currency}*.`, m)
