...
async function sticker(img, url, ...args) {
    let lastError;
    let stiker;
    for (const func of [support.ffmpeg && sticker6, sticker5].filter((f) => f)) {
        try {
            stiker = await func(img, url, ...args);
            if (Buffer.isBuffer(stiker) && stiker.toString().includes("html")) continue;
            if (Buffer.isBuffer(stiker) || (typeof stiker === 'string' && stiker.includes("WEBP"))) {
                try {
