        ...
        getName: {
            async value(jid = '', withoutContact = false) {
                try {
                    ...
                    jid = String(jid).split(':')[0].trim()

                    if (jid.endsWith?.('@lid')) {
                        const cache = this.lidToPnCache || globalThis.lidToPnCache
                        if (cache?.has?.(jid)) {
                            const pn = cache.get(jid)
                            if (pn?.endsWith?.('@s.whatsapp.net')) {
                                jid = pn.split(':')[0]
                            }
                        } else {
                            try {
                                ...
                                if (pn?.endsWith?.('@s.whatsapp.net')) {
                                    ...
                                    this.lidToPnCache.set(jid, cleanPn)
                                    jid = cleanPn
                                }
                            } catch {}
                        }
                    ...
                    if (jid === this.user?.jid || jid === this.user?.id) {
                        return this.user?.name || this.user?.verifiedName || 'Bot'
                    }
                    ...
                    for (const variant of jidVariants) {
                        const chat = this.chats?.[variant] || {}
                        const contact = this.contacts?.[variant] || {}
                        ...
                        if (nameFromDb) return nameFromDb
                    }


                    try {
                        for (const g of Object.values(this.chats || {})) {
                            if (!g?.metadata?.participants) continue
                            const p = g.metadata.participants.find(pp =>
                                pp.id?.split?.(':')[0] === jid ||
                                pp.lid?.split?.(':')[0] === jid ||
                                pp.phoneNumber?.split?.(':')[0] === jid
                            )
                            if (p) {
...
export function smsg(conn, m, hasParent) {
    ...
    try {
        m.senderPn = m.key?.participantAlt || m.key?.participantPn || m.participantAlt || null
        m.senderLid = m.key?.participantLid || m.key?.participant || null

        const rawParticipant = m.key?.participant || m.participant
        if (rawParticipant?.endsWith?.('@lid')) {
            m.senderLid = rawParticipant
            m.senderPn = m.key?.participantAlt || null
        } else if (rawParticipant?.endsWith?.('@s.whatsapp.net')) {
            m.senderPn = rawParticipant
        }

        if (m.key?.remoteJidAlt && !m.senderPn) {
            m.senderPn = m.key.remoteJidAlt
        }
    } catch {}
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                if (this.key?.fromMe) {
                    return (this.conn?.user?.id || this.conn?.user?.jid || '').split(':')[0]
                }
                ...
                const pn = this.key?.participantAlt || this.senderPn
                if (raw.endsWith?.('@lid') && pn?.endsWith?.('@s.whatsapp.net')) {
                    return pn.split(':')[0]
                }
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    sender: {
                        get() {
                            ...
                            const pn = contextInfo.participantAlt
                            if (raw.endsWith?.('@lid') && pn?.endsWith?.('@s.whatsapp.net')) {
                                return pn.split(':')[0].decodeJid()
                            }
                            return raw.split(':')[0].decodeJid()
                        },
                        enumerable: true
                    },
                    fromMe: {
