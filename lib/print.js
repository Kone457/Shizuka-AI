...
// Importación de módulos condicionales
...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(`${chalk.hex('#FE0041').bold('╭─〔 🌸 𝙎𝙝𝙞𝙯𝙪𝙠𝙖 · 𝘼𝙄 〕─╮')}
${chalk.hex('#00BFFF').bold('│')}${chalk.redBright('Bot:')} ${chalk.greenBright(me)} ~ ${chalk.magentaBright(userName)} ${global.conn.user.jid === conn.user.jid ? chalk.cyanBright('(Principal)') : chalk.cyanBright('(Sub-Bot)')}
...
${chalk.hex('#00BFFF').bold('│')}${chalk.greenBright('Tipo de evento:')} ${chalk.redBright(m.messageStubType ? WAMessageStubType[m.messageStubType] : 'Ninguno')}
...
${chalk.hex('#00BFFF').bold('│')}${chalk.blueBright('Remitente:')} ${chalk.redBright(sender)}
${chalk.hex('#00BFFF').bold('│')}${chalk.cyanBright(`Chat ${m.isGroup ? 'Grupal' : 'Privado'}:`)} ${chalk.greenBright(chat)}
${chalk.hex('#00BFFF').bold('│')}${chalk.magentaBright('Tipo de mensaje:')} ${chalk.yellowBright(m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg?.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : 'Desconocido')}
